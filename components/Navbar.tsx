...
const Navbar = () => {
  ...
  return (
    <nav className="bg-ledger sticky top-0 z-50 border-b-2 border-ink-900">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
              ...
              <div key={item.href || item.label} className="relative">
                {item.href ? (
                  ...
                  <div className="relative">
                    ...
                    <div
                      className={`absolute left-0 mt-2 w-56 bg-ledger border-2 border-ink-900 py-1 z-50 ${openDropdown === item.label ? 'block' : 'hidden'}`}
                    ...
                    >
                      {item.subItems?.map((subItem) => (
                        <Link
                          key={subItem.href}
                          href={subItem.href}
                          className="block px-4 py-2.5 font-mono text-xs uppercase tracking-wide text-ink-700 hover:bg-ink-900 hover:text-ledger"
            ...
            <Link
              href="/register"
              className="btn-press ml-3 bg-stamp-600 text-ledger px-5 py-2.5 font-mono text-xs font-semibold uppercase tracking-wide hover:bg-stamp-700"
            >
              Register
            </Link>
          </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden bg-ledger border-t-2 border-ink-900 overflow-hidden"
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                ...
                <div key={item.href || item.label}>
                  {item.href ? (
                    ...
                    <div className="space-y-1">
                      ...
                      <div className={`pl-4 border-l-2 border-ledger-line ${openDropdown === item.label ? 'block' : 'hidden'}`}>
                        {item.subItems?.map((subItem) => (
                          <Link
                            key={subItem.href}
                            href={subItem.href}
              ...
              <Link
                href="/register"
                className="btn-press block text-center bg-stamp-600 text-ledger px-3 py-3 font-mono text-xs font-semibold uppercase tracking-wide"
                onClick={() => setIsOpen(false)}
              >
                Register
              </Link>
